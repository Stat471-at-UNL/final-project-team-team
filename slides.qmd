...
- Want to see whether this different mapping of coordinate data has an effect on imputation and creating a model

- By conducting imputation using latitude/longitude and then again with UTM, we can compare the coordinate systems for imputation before modeling
...
$$\beta_5(\text{latitude} \times \text{longitude}) + \beta_6(\text{accommodates})$$
