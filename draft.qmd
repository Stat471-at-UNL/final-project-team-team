...
variables <- names(filter_df)
variables
```

### Data Description

The variables that we deemed important were `r variables`. Each of these variables are either factors, continuous, and something that we found could be useful in the analysis.

The `price` variable is our variable of interest since we found that several observations had missing price variables. Based on other variables such as `beds`, `bathrooms`, `property_type`, and `neighborhood`, we believe that we could accurately predict the missing prices for NYC Airbnbs.
