...
The `price` variable describes the daily price for a listing in its local currency. Since all listings in this analysis are from New York City, all prices are in \$USD. There are `r round(sum(!is.na(filter_df$price)), 2)` observations of price where it has a numerical value and it is not missing from the data.
...
There are `r sum(is.na(filter_df$price))` missing values in `price`. Which means missing values make up around `r paste0(round(sum(is.na(filter_df$price)) / nrow(filter_df), 4) * 100, "%")` of the observations in the dataset.
...
*This was supposed to be a `callout` but it had problems rendering as a pdf*
Taking a look at listings with 0 beds, and I found that there are some like this:
...
ggplot(aes(x = beds)) + 
  ...
  labs(
    ...
    # subtitle = "Only Measuring Observations Where `beds` < 40",
    x = "Number of Beds",
    y = "Count"
  )
```
