...
library(rapportools) 
library(knitr)
library(kableExtra)# added, couldn't render is.empty() w/out it
source("data_pull.R")
```
...
A histogram of `price` shows that the distribution is very right-skewed, as most of the dataset has listings in the $\le\$1000$ price range. In fact only, `r paste0(round(price_range_table$n[2] / nrow(filter_df), 4) * 100, "%")` of the dataset (including NA values) has Airbnb listings over the $\$1000$ price range.
...
Similarly to `price`, there are `r sum(is.na(filter_df$beds))` missing values in the `beds` variable, that is `r paste0(round(sum(is.na(filter_df$beds)) / nrow(filter_df), 4) * 100, "%")` of observations in the dataset.
...
<<<<<<< HEAD
<!-- ### Counts of Number of Beds Table -->
...
=======
**COMMENT 2: I took out the tables, I think the histogram now does the job, it still would be good to discuss outliers the same however.**
>>>>>>> 5c96e31971cf0db4e62f00571f595d400030916a
...
\begin{align}
\log(\text{Price}) &= \beta_0 + \beta_1 \cdot \text{bedrooms} + \beta_2 \cdot \text{beds} + \beta_3 \cdot \text{latitude} \\
&+ \beta_4 \cdot \text{longitude} + \beta_5 \cdot (\text{latitude} \times \text{longitude}) \\
&+ \beta_6 \cdot \text{accommodates} + \beta_7 \cdot \text{bedrooms}
\end{align}
...
```{r}
#| label: model_comp
#| tbl-cap: Comparing Model Fit Between Geographic vs. UTM
#| tbl-pos: hold
...
The anova test above shows that we fail to reject the null hypothesis, (p - value > .05), which is that the predictor sets mean RMSE values are equal. This shows that there is no statistical difference in using the predictor set with geographic coordinates or UTM in regards to imputation quality. It can be said that neither the imputation or the model would directly benefit from using UTM over geographic coordinates.
