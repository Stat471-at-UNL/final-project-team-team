...
$$ \beta_5(\text{latitude x longitude}) + \beta_6(\text{accommodates}) + \beta_7(\text{bathrooms})$$
...
```{r}
#| label: summary-statistisc
#| tbl-cap: Summary Statistics for Outcome `Price` and Predictors 
#| tbl-pos: hold
filter_df %>%
  select(price, beds, bedrooms, bathrooms, accommodates) %>%
  summary() %>%
